        ...
        <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-sm-between gap-3">
            ...
            <div class="d-flex gap-2">
                @if (Auth::user()->canReviewLogbook())
                    <a href="{{ route('admin.logbook.export') }}" class="btn btn-success btn-sm rounded-pill px-3"><i class="bi bi-download me-1"></i>Export CSV</a>
                @endif
                @if (Auth::user()->anggota_id)
                    <a href="{{ route('admin.logbook.create') }}" class="btn btn-primary btn-sm rounded-pill px-3">
                        ...
                        <tr>
                            ...
                            <td class="pe-4 py-3 text-end">
                                @if ($logbook->isEditableBy(Auth::user()))
                                    <a href="{{ route('admin.logbook.edit', $logbook) }}" class="btn btn-sm btn-light rounded-circle"><i class="bi bi-pencil-square"></i></a>
                                @endif
                            ...
                            <div class="modal fade" id="review-{{ $logbook->id }}" tabindex="-1">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form method="POST" action="{{ route('admin.logbook.review', $logbook) }}">
                                            @csrf
                                            @method('PATCH')
