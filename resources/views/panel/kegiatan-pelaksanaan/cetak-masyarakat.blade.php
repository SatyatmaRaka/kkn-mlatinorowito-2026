...
<head>
    <meta charset="utf-8">
    <title>Daftar Hadir Peserta/Masyarakat</title>
    <style>
...
<body>
    ...
    <h1>Daftar Hadir Peserta/Masyarakat</h1>
    ...
    <table>
        <thead><tr><th>No</th><th>Nama</th><th>Alamat</th><th>Tanda Tangan</th></tr></thead>
        <tbody>
        @foreach ($kegiatan->pesertaMasyarakat as $p)
            <tr><td style="text-align:center">{{ $loop->iteration }}</td><td>{{ $p->nama }}</td><td>{{ $p->alamat ?? '' }}</td><td class="ttd"></td></tr>
        @endforeach
        </tbody>
    </table>
    <div class="footer">
