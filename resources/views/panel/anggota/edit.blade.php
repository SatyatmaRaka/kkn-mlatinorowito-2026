<x-app-layout>
    ...
    <div class="card shadow-sm">
        <div class="card-body p-4">
            <form action="{{ route('panel.anggota.update', $anggota->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select id="jabatan" name="jabatan" class="form-select @error('jabatan') is-invalid @enderror" required>
                        @foreach (\App\Enums\Jabatan::cases() as $jabatanOption)
                            <option value="{{ $jabatanOption->value }}" @selected(old('jabatan', $anggota->jabatan) === $jabatanOption->value)>
                                {{ $jabatanOption->value }}
                            </option>
                        @endforeach
                    </select>
